...
- Process records, file descriptors, async `spawn`, blocking `wait` and `sleep`, deep-copy `fork`, and `exec`/`execv`/`execve`, including shebang scripts with bounded recursion.
