...
![Version](https://img.shields.io/badge/version-0.0.1.12-2f6fed)
...
The current release line is `0.0.1.12`. At this point AuroraOS can boot through a BIOS stage1/stage2 loader, enter long mode, run a protected kernel, mount in-memory and disk filesystems, enter ring3, execute small ELF64 user programs, preempt them on a single CPU, and replace a process image through `exec`/`execv`.
...
| Filesystems | VFS, writable ramfs, devfs, tarfs, read-only EXT4 adapter |
...
scripts/               build and contract-check scripts
