...
static block_status_t ata_flush(bool ext) {
    ...
    return ata_wait_not_bsy() ? BLOCK_OK : BLOCK_ERR_TIMEOUT;
}
...
static block_status_t ata_write(block_device_t *dev, u64 lba, u32 count, const void *buffer) {
    ...
    if (lba < 0x10000000ull && (u64)count <= 0x10000000ull - lba) return ata_write28(dev, lba, count, buffer);
    if (!primary_supports_lba48) return BLOCK_ERR_RANGE;
    return ata_write48(dev, lba, count, buffer);
}
