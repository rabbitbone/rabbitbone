...
typedef struct AURORA_PACKED acpi_rsdp_v1 {
    char signature[8];
    u8 checksum;
    char oem_id[6];
    u8 revision;
...
typedef struct AURORA_PACKED acpi_rsdp_v2 {
    acpi_rsdp_v1_t v1;
    u32 length;
    u64 xsdt_address;
...
typedef struct AURORA_PACKED acpi_sdt_header {
    char signature[4];
    u32 length;
    u8 revision;
    u8 checksum;
...
typedef struct AURORA_PACKED acpi_madt {
    acpi_sdt_header_t h;
    u32 lapic_address;
    u32 flags;
    u8 entries[];
...
typedef struct AURORA_PACKED acpi_madt_entry_header {
    u8 type;
    u8 length;
} acpi_madt_entry_header_t;

typedef struct AURORA_PACKED acpi_madt_lapic {
...
typedef struct AURORA_PACKED acpi_madt_ioapic {
    acpi_madt_entry_header_t h;
    u8 ioapic_id;
    u8 reserved;
    u32 ioapic_address;
...
typedef struct AURORA_PACKED acpi_madt_iso {
    acpi_madt_entry_header_t h;
    u8 bus;
    u8 source;
    u32 gsi;
...
typedef struct AURORA_PACKED acpi_madt_lapic_override {
    acpi_madt_entry_header_t h;
    u16 reserved;
    u64 lapic_address;
} acpi_madt_lapic_override_t;

typedef struct AURORA_PACKED acpi_gas {
    u8 address_space_id;
...
typedef struct AURORA_PACKED acpi_hpet_table {
    acpi_sdt_header_t h;
...
static const acpi_rsdp_v2_t *scan_rsdp_range(uptr start, uptr end) {
    for (uptr p = AURORA_ALIGN_UP(start, 16u); p + sizeof(acpi_rsdp_v1_t) <= end; p += 16u) {
        ...
        if (r->v1.revision >= 2u) {
            ...
            if (!checksum_ok(r, r->length)) continue;
        }
        return r;
    }
    return 0;
...
static bool valid_sdt(const acpi_sdt_header_t *h) {
    if (!h || h->length < sizeof(*h) || h->length > (1024u * 1024u)) return false;
    return checksum_ok(h, h->length);
}

static void add_table(const acpi_sdt_header_t *h) {
    if (!valid_sdt(h) || acpi_table_count >= AURORA_ARRAY_LEN(acpi_tables)) return;
...
static void parse_root_table(const acpi_sdt_header_t *root, bool xsdt) {
    ...
    if (xsdt) {
        ...
        for (usize i = 0; i < entries; ++i) add_table((const acpi_sdt_header_t *)(uptr)p[i]);
    } else {
        entries = (root->length - sizeof(*root)) / sizeof(u32);
        const u32 *p = (const u32 *)((const u8 *)root + sizeof(*root));
        for (usize i = 0; i < entries; ++i) add_table((const acpi_sdt_header_t *)(uptr)(u64)p[i]);
    }
...
static void parse_madt(void) {
    ...
    while (p + sizeof(acpi_madt_entry_header_t) <= end) {
        ...
        if (eh->type == 0 && eh->length >= sizeof(acpi_madt_lapic_t)) {
            ...
            acpi_info.lapic_address = (u32)e->lapic_address;
        }
        p += eh->length;
    }
}
...
void acpi_format_status(char *out, usize out_len) {
    ...
    aurora_buf_appendf(&bo, "acpi: found=%u rsdp=%p root=%p xsdt=%u tables=%u\n",
            acpi_info.found ? 1u : 0u, (void *)(uptr)acpi_info.rsdp, (void *)(uptr)acpi_info.root_table,
            acpi_info.xsdt ? 1u : 0u, acpi_info.table_count);
    aurora_buf_appendf(&bo, "  madt=%u lapic=%p cpus=%u enabled=%u ioapics=%u iso=%u\n",
