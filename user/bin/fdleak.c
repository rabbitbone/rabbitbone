...
int main(int argc, char **argv) {
    ...
    const int handle_count = (int)AURORA_PROCESS_HANDLE_CAP - 3;
    const int last = handle_count - 1;
    const int prev = handle_count - 2;
    const au_i64 last_handle = (au_i64)(AURORA_PROCESS_HANDLE_CAP - 1u);
    const au_i64 prev_handle = (au_i64)(AURORA_PROCESS_HANDLE_CAP - 2u);
    ...
    if (au_dup2(fds[0], last_handle, AURORA_FD_CLOEXEC) != last_handle) return 95;
    au_memset(&info, 0, sizeof(info));
    if (au_fdinfo(last_handle, &info) != 0 || info.handle != (unsigned int)last_handle || !(info.flags & AURORA_FD_CLOEXEC)) return 96;
