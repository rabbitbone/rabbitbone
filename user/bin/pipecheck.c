...
static int cat_to_stdout(const char *path) {
    au_i64 in = path && !(path[0] == '-' && path[1] == 0) ? au_open(path) : (au_i64)AURORA_STDIN;
    if (in < 0) return 70;
    au_fdinfo_t out_info;
    au_memset(&out_info, 0, sizeof(out_info));
    if (au_fdinfo((au_i64)AURORA_STDOUT, &out_info) != 0 || out_info.handle != AURORA_STDOUT) return 71;
    char buf[128];
    for (;;) {
    ...
    if (in > (au_i64)AURORA_STDERR && au_close(in) != 0) return 74;
    return 0;
}
...
static int check_stdio_pipeline(void) {
    ...
    if (au_fdinfo((au_i64)AURORA_STDIN, &stdio) != 0 || stdio.handle != AURORA_STDIN) return 40;
    if (au_fdinfo((au_i64)AURORA_STDOUT, &stdio) != 0 || stdio.handle != AURORA_STDOUT) return 41;
    if (au_fdinfo((au_i64)AURORA_STDERR, &stdio) != 0 || stdio.handle != AURORA_STDERR) return 42;
    ...
    if (child == 0) {
        ...
        if (au_dup2((au_i64)stdfds[1], (au_i64)AURORA_STDOUT, 0) != (au_i64)AURORA_STDOUT) return 46;
        if (au_fdctl((au_i64)stdfds[1], AURORA_FDCTL_SET, AURORA_FD_CLOEXEC) != AURORA_FD_CLOEXEC) return 47;
...
int main(int argc, char **argv) {
    ...
    if (fds[0] <= AURORA_STDERR || fds[1] <= AURORA_STDERR || fds[0] == fds[1]) return 11;
    au_pipeinfo_t pi;
    au_memset(&pi, 0, sizeof(pi));
    if (au_pipeinfo(fds[0], &pi) != 0) return 12;
    if (pi.capacity != AURORA_PIPE_BUF || pi.endpoint != 1u || pi.pipe_id == 0 || pi.read_refs != 1u || pi.write_refs != 1u) return 13;
    const char msg[] = "aurora-pipe-ipc";
    ...
    au_i64 hup = au_poll((au_i64)hupfds[0], AURORA_POLL_READ | AURORA_POLL_HUP);
    if ((hup & AURORA_POLL_HUP) == 0) return 33;
    ...
    au_i64 whup = au_poll((au_i64)b[1], AURORA_POLL_WRITE | AURORA_POLL_HUP);
    if ((whup & AURORA_POLL_HUP) == 0) return 57;
    ...
    if (au_fdctl((au_i64)forkfds[0], AURORA_FDCTL_SET, AURORA_FD_CLOEXEC) != AURORA_FD_CLOEXEC) return 28;
    au_fdinfo_t fi;
    au_memset(&fi, 0, sizeof(fi));
    if (au_fdinfo((au_i64)forkfds[0], &fi) != 0 || !(fi.flags & AURORA_FD_CLOEXEC)) return 29;
